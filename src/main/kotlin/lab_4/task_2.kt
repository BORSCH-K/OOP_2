...
//    prettyPrintCursor(mCourses.find(Course::name eq "Math"))
    ...
    addStudent(
        "Emily", "Girls", listOf(
            "Math",
            "Chem"  // на химию не записан, так как ее нет в списках
        )
    )
...
fun addStudent(name: String, group: String, courses: List<String>) {
    ...
    mCourses.updateMany(Course::name `in` courses, push(Course::grades, Grade(student.id, name)))
